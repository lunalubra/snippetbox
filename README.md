...
DSN `web:acosta@/snippetbox?parseTime=true`. The schema in `db/schema.sql` can
be applied to a local database too.
...
- `.environment` — builds the `DSN` environment variable from the credentials
  Upsun injects for the `mysql` relationship (`MYSQL_*`, falling back to
  decoding `$PLATFORM_RELATIONSHIPS`).
